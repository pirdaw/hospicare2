                ...
                <div class="card shadow mb-4">
                    ...
                    <div class="card-body">
                        <table class="table table-bordered">
                            <tr><th width="200">Nama</th><td>{{ $pasien->nama }}</td></tr>
                            <tr><th>NIK</th><td><code>{{ $pasien->nik }}</code></td></tr>
                            <tr><th>Umur</th><td>{{ $pasien->umur }} tahun</td></tr>
                            <tr><th>Jenis Kelamin</th><td>{{ $pasien->jenis_kelamin === 'L' ? 'Laki-laki' : 'Perempuan' }}</td></tr>
                            <tr><th>Golongan Darah</th><td>{{ $pasien->golongan_darah ?? '-' }}</td></tr>
                            <tr><th>Agama</th><td>{{ $pasien->agama }}</td></tr>
                            <tr><th>Pekerjaan</th><td>{{ $pasien->pekerjaan }}</td></tr>
                            <tr><th>No HP</th><td>{{ $pasien->no_hp }}</td></tr>
                            <tr><th>Alamat</th><td>{{ $pasien->alamat }}</td></tr>
                            <tr><th>Terdaftar</th><td>{{ $pasien->created_at->format('d M Y') }}</td></tr>
                        </table>
                    </div>
                </div>
                                ...
                                <table class="table table-bordered table-hover">
                                    ...
                                    <tbody>
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                <span class="badge badge-{{ $k->status === 'selesai' ? 'success' : ($k->status === 'diperiksa' ? 'info' : 'warning') }}">
                                                    {{ ucfirst($k->status) }}
                                                </span>
                                            </td>
                                            <td>
